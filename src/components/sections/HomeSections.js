...
// --- 2. SectionHeader — text-white → text-gray-900 ، text-gray-400 → text-gray-500 ---
  ...
  <div className="flex items-center justify-between mb-6 px-4 pt-10" dir="rtl">
    <div className="flex items-center gap-3">
      ...
      <div>
        ...
        {subTitle && <p className="text-gray-500 text-[10px] md:text-xs mt-1 font-normal">{subTitle}</p>}
      </div>
    </div>
...
export const FeaturedToday = ({ data }) => {
  ...
  return (
    // bg-[#181818] → bg-white ، border-[#333] → border-gray-200
    <section className="bg-white pt-2 pb-1 mt-0 border-t border-gray-200">
      <GlobalStyles />
      <div className="max-w-[1400px] mx-auto relative px-4 text-right">
        ...
        <div className="relative group/slider">
          ...
          {/* أسهم التنقل — bg-black/40 → bg-gray-900/40 ، border-white/20 → border-gray-300 ، text-white → text-gray-700 */}
          <button 
            onClick={() => scroll('right')}
            className={`absolute top-1/2 -translate-y-1/2 right-2 z-30 bg-gray-900/40 hover:bg-gray-900/60 border border-gray-300 text-white w-10 h-14 flex items-center justify-center backdrop-blur-sm transition-all duration-300 ${showRightArrow ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
          >
          ...
          <button 
            onClick={() => scroll('left')}
            className={`absolute top-1/2 -translate-y-1/2 left-2 z-30 bg-gray-900/40 hover:bg-gray-900/60 border border-gray-300 text-white w-10 h-14 flex items-center justify-center backdrop-blur-sm transition-all duration-300 ${showLeftArrow ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
          >
                ...
                <div key={mIndex} className="flex items-stretch snap-start">
                  ...
                  <div className="relative z-50 flex flex-col gap-2 pb-2">
                    <Link href={mainCard.linkUrl || "#"} className="min-w-[150px] md:min-w-[190px] block relative">
                      {/* bg-[#222] → bg-gray-200 ، border-[#333] → border-gray-200 */}
                      <div className={`relative aspect-[2/3] overflow-hidden bg-gray-200 shadow-md transition-all duration-500 ${isExpanded && hasSubCards ? 'rounded-none border-l border-gray-200' : 'rounded-2xl'}`}>
                        <img src={mainCard.image} alt={mainCard.mainTitle} className="w-full h-full object-cover" />
                        {/* from-[#121212]/90 → from-gray-900/70 */}
                        <div className="absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-t from-gray-900/70 to-transparent pointer-events-none"></div>
                    ...
                    <div className="px-1 text-right mt-1 flex justify-between items-start">
                      <div>
                        {/* text-white → text-gray-900 */}
                        <h3 className="text-gray-900 text-[13px] md:text-sm font-bold line-clamp-2">{mainCard.mainTitle}</h3>
                        {/* text-[#5799ef] يفضل */}
                        <span className="text-[#5799ef] text-[10px] md:text-xs font-semibold">{mainCard.linkText}</span>
                      </div>
                      
                      {hasSubCards && !isExpanded && (
                        // bg-[#222] → bg-gray-100 ، border-[#444] → border-gray-300 ، text-white → text-gray-700
                        <button 
                          onClick={(e) => toggleDeck(mIndex, e)}
                          className="bg-gray-100 hover:bg-gray-200 border border-gray-300 text-gray-700 p-1.5 rounded-full transition-all duration-300 ml-1 shrink-0"
                        >
                  ...
                  {hasSubCards && mainCard.subCards.map((subCard, sIndex) => {
                    ...
                    return (
                      ...
                      >
                        {/* bg-[#222] → bg-gray-200 ، border-r border-[#181818]/60 → border-r border-gray-200 ، shadow داكن → فاتح */}
                        <div className="relative aspect-[2/3] rounded-none overflow-hidden bg-gray-200 border-r border-gray-200 shadow-md">
                        ...
                        <div className={`px-1 text-right mt-1 flex justify-between items-start transition-opacity duration-500 ${isExpanded ? 'opacity-100' : 'opacity-0'}`}>
                          <div>
                            {/* text-white → text-gray-900 */}
                            <h3 className="text-gray-900 text-[13px] md:text-sm font-bold line-clamp-2">{subCard.mainTitle}</h3>
                            <span className="text-[#5799ef] text-[10px] md:text-xs font-semibold">{subCard.linkText}</span>
                          </div>
                          
                          {isExpanded && isLastSubCard && (
                            // bg-[#222] → bg-gray-100 ، border-[#444] → border-gray-300
                            <button 
                              onClick={(e) => toggleDeck(mIndex, e)}
                              className="bg-gray-100 hover:bg-gray-200 border border-gray-300 text-[#F5C518] p-1.5 rounded-full transition-all duration-300 ml-1 shrink-0"
                            >
...
export const TopTenProducts = ({ data }) => {
  ...
  return (
    // bg-[#000000] → bg-[#f5f5f0] ، border-[#333] → border-gray-200
    <section className="bg-[#f5f5f0] pt-6 pb-10 mt-0 border-t border-gray-200">
      <div className="max-w-[800px] mx-auto relative px-4 text-right" dir="rtl">
        
        {/* text-white → text-gray-900 ، group-hover:text-[#F5C518] يفضل */}
        <div className="flex items-center gap-2 mb-6 cursor-pointer group w-max">
          <div className="w-1.5 h-6 bg-[#F5C518] rounded-sm"></div>
          <h2 className="text-xl md:text-2xl font-black text-gray-900 tracking-tight flex items-center gap-1 transition-colors group-hover:text-[#F5C518]">
            {data.title || "أفضل 10 منتجات لدي وينـد"}
            {/* text-white → text-gray-700 */}
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 md:h-6 md:w-6 mt-1 text-gray-700 group-hover:text-[#F5C518]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="flex flex-col gap-4">
          {data.cards.slice(0, 10).map((card, index) => (
            // bg-[#1a1a1a] → bg-white ، border-[#222] → border-gray-200 ، hover:border-[#444] → hover:border-gray-400
            <div key={index} className="flex bg-white rounded-xl overflow-hidden shadow-sm border border-gray-200 transition-all hover:border-gray-400 hover:shadow-md">
              
              {/* bg-[#222] → bg-gray-100 */}
              <div className="relative w-[110px] md:w-[130px] shrink-0 bg-gray-100">
                <img src={card.image} alt={card.mainTitle} className="w-full h-full object-cover aspect-[2/3]" />
                
                {/* bg-black/60 يفضل — على الصورة */}
              ...
              <div className="flex-1 p-3 md:p-4 flex flex-col justify-start">
                ...
                {/* الـ Ranking Badge — لم يتغير (أزرق) */}
                <div className="mb-2">
                ...
                {/* text-white → text-gray-900 */}
                <h3 className="text-gray-900 font-bold text-base md:text-lg mb-1 line-clamp-2 leading-tight">{card.mainTitle}</h3>
                
                {/* text-gray-400 → text-gray-500 ، text-white → text-gray-900 */}
                <div className="flex items-center gap-3 text-gray-500 text-[11px] md:text-sm mb-2">
                  <span className="text-gray-900 font-semibold">{card.price || "متوفر الآن"}</span>
                ...
                <div className="flex items-center gap-3 mb-3">
                  <div className="flex items-center gap-1">
                    ...
                    {/* text-white → text-gray-900 ، text-gray-400 → text-gray-500 */}
                    <span className="text-gray-900 text-[13px] md:text-sm font-bold">{card.rating || "5.0"}</span>
                    <span className="text-gray-500 text-[11px] md:text-xs">({card.reviewsCount || "التقييمات"})</span>
                  </div>
                  {/* hover:bg-[#2a3f5f] → hover:bg-blue-50 ، text-[#5799ef] يفضل */}
                  <button className="flex items-center gap-1 text-[#5799ef] hover:bg-blue-50 px-2 py-0.5 rounded transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="mt-auto">
                  {/* text-[#5799ef] يفضل ، hover:text-white → hover:text-gray-900 */}
                  <Link href={card.linkUrl || "/"} className="inline-flex items-center gap-2 text-[#5799ef] hover:text-gray-900 transition-colors text-[13px] md:text-sm font-bold">
        ...
        {/* bg-[#242424] → bg-white ، border-[#333] → border-gray-200 ، text-[#5799ef] يفضل ، hover:bg-[#333] → hover:bg-gray-50 */}
        <div className="mt-6 flex justify-center pb-2">
          <Link href={data.linkUrl || "/"} className="w-full text-center bg-white hover:bg-gray-50 text-[#5799ef] font-bold py-3 px-8 rounded-full transition-colors text-sm md:text-base border border-gray-200">
            عرض الكل
          </Link>
        </div>
      </div>
