...
// تأكد أن المسار هنا مطابق لمكان ملف الـ Context عندك
import { useCart } from "../../context/CartContext";

export default function Navbar() {
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  // تأمين: في حال كانت السلة غير محملة بعد، نضع قيمة افتراضية
  ...
  return (
    <nav className="bg-[#121212] border-b border-[#333] sticky top-0 z-[100] h-14 w-full">
      <div className="max-w-[1280px] mx-auto px-4 h-full flex items-center justify-between">
        
        {/* زر القائمة الجانبية */}
        <button 
          onClick={() => setIsMenuOpen(true)} 
          className="text-white p-1 active:scale-90 transition-transform flex-shrink-0"
        >
        ...
        <div className="fixed inset-0 z-[150] flex" dir="rtl">
          ...
          <div className="relative bg-[#1a1a1a] w-3/4 max-w-[300px] h-full shadow-2xl flex flex-col border-l border-[#333]">
            <div className="p-4 bg-[#222] flex justify-between items-center">
              ...
              <button onClick={() => setIsMenuOpen(false)} className="text-white text-xl p-2">✕</button>
            </div>
