...
export default function CustomersPage() {
  ...
  const exportToExcel = () => {
    ...
    const headers = ["الاسم,الإيميل,إجمالي الإنفاق,عدد الطلبات,المدينة"];
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          <div className="lg:col-span-1 space-y-2">
            <p className="text-xs font-bold text-gray-400 mb-4 px-2 uppercase">الشرائح (Segments)</p>
          ...
          <div className="lg:col-span-3 space-y-4 bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="p-4 border-b border-gray-100 relative">
              ...
              <input type="text" placeholder="ابحث بالاسم أو الإيميل..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pr-10 pl-4 py-3 bg-gray-50 border rounded-lg text-sm outline-none focus:border-[#008060]" />
            </div>

            <div className="overflow-x-auto">
              <table className="w-full text-right">
                <thead className="bg-gray-50 text-[11px] font-bold text-gray-400"><tr><th className="px-6 py-4">العميل</th><th className="px-6 py-4">الموقع</th><th className="px-6 py-4 text-center">الطلبات</th><th className="px-6 py-4">الإنفاق</th></tr></thead>
                <tbody className="divide-y divide-gray-100">
                  {loading ? <tr><td colSpan="4" className="text-center py-20 text-gray-400">جاري التحميل...</td></tr> : 
                    customers.filter(c => c.Email?.includes(search) || c['First Name']?.includes(search)).map((c) => {
                      // 🔥 الحل الجذري لمشكلة الـ undefined: نأخذ الـ ID الخاص بالمستند لو الإيميل مش موجود
                      const safeId = c.Email || c.id; 
                      return (
                        <tr key={c.id} className="hover:bg-gray-50 cursor-pointer group" onClick={() => router.push(`/admin/customers/${encodeURIComponent(safeId)}`)}>
                          <td className="px-6 py-4"><p className="text-sm font-bold text-[#005bd3]">{c['First Name']} {c['Last Name']}</p><p className="text-[10px] text-gray-400">{c.Email || 'بدون إيميل'}</p></td>
                          <td className="px-6 py-4 text-xs text-gray-600">{c['Default Address City'] || '---'}</td>
