...
export default function AdminHomeManager() {
  ...
  // هذا المكون هو المسؤول عن رسم الشكل المصغر بدقة
  const WINDVisualMockup = ({ section }) => {
    ...
    return (
      <div className="w-full bg-[#151515] rounded-3xl p-6 border border-white/10 shadow-[0_10px_40px_rgba(0,0,0,0.5)] space-y-5 overflow-hidden transition-all duration-300 group hover:border-[#F5C518]/30">
        ...
        {/* الهيدر الأصلي (بنفس مقاسات الموقع) */}
        ...
        {/* منطقة المحتوى (الاستايلات) */}
        <div className="px-1 min-h-[140px] flex flex-col justify-center relative">
            
            {/* 1. ستايل الأكثر مبيعاً (1/3 صورة كبيرة & 2/3 شبكة) */}
            {layout === 'bestseller_split' && (
                <div className="flex gap-3 h-40 w-full">
                    ...
                    <div className="w-2/3 grid grid-cols-2 gap-2">
                        {[...Array(4)].map((_, i) => (
                            <div key={i} className="bg-[#222] rounded-xl border border-white/5 relative">
                                <div className="absolute bottom-2 right-2 w-8 h-1 bg-white/10 rounded-full"></div>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            {/* 2. ستايل الماركي اللانهائي (شريط متحرك) */}
            ...
            {/* 3. ستايل شريط الثقة (أيقونات) */}
            {layout === 'trust_bar' && (
                <div className="bg-[#1a1a1a] py-6 border-y border-white/10 flex justify-evenly items-center px-4 rounded-xl">
                    {[...Array(3)].map((_, i) => (
                        <div key={i} className="flex flex-col items-center gap-2">
                            <div className="w-10 h-10 rounded-full bg-[#111] border border-white/10 flex items-center justify-center text-[#F5C518] text-xs font-black">
                                ★
                            </div>
            ...
            {/* 4. ستايل المجلة (مقالات) */}
            {layout === 'magazine_grid' && (
                <div className="grid grid-cols-2 gap-3 h-36">
                    <div className="bg-[#222] rounded-2xl relative overflow-hidden border border-white/5 group">
                        <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent"></div>
                        <div className="absolute bottom-3 right-3 w-16 h-2 bg-[#F5C518] rounded-sm mb-2"></div>
                        <div className="absolute bottom-8 right-3 w-3/4 h-2 bg-white/50 rounded-sm"></div>
                    </div>
                    <div className="bg-[#222] rounded-2xl relative overflow-hidden border border-white/5 group">
                        <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent"></div>
                        <div className="absolute bottom-3 right-3 w-16 h-2 bg-[#F5C518] rounded-sm mb-2"></div>
            ...
            {/* 5. ستايل آراء العملاء (شريط أفقي) */}
                    ...
                    {[...Array(3)].map((_, i) => (
                        <div key={i} className="min-w-[160px] bg-[#222] p-3 rounded-2xl border border-white/5 flex flex-col gap-2">
                            <div className="flex items-center gap-2">
                                <div className="w-6 h-6 rounded-full bg-[#F5C518] flex items-center justify-center text-black text-[8px] font-bold">U</div>
            ...
            {layout === 'story_banner' && (
                <div className="h-40 bg-[#111] rounded-[2rem] flex flex-col items-center justify-center p-6 text-center border border-white/10 relative overflow-hidden">
                    ...
                    <div className="w-1/2 h-1.5 bg-white/20 rounded-full relative z-10"></div>
                </div>
            )}

            {/* 7. ستايل الشبكة الافتراضي */}
            {layout === 'grid_default' && (
                <div className="grid grid-cols-4 gap-2">
                    {[...Array(4)].map((_, i) => (
                        <div key={i} className="aspect-[3/4] bg-[#222] rounded-xl border border-white/5 flex flex-col justify-end p-2">
            ...
            {/* 8. ستايل بوسترات IMDb */}
            {layout === 'imdb_posters' && (
                <div className="flex gap-3 overflow-hidden">
                    {[...Array(5)].map((_, i) => (
                        <div key={i} className="min-w-[90px] aspect-[2/3] bg-[#222] rounded-2xl border border-white/5 relative shadow-lg">
                             <div className="absolute bottom-2 right-2 w-8 h-1.5 bg-[#F5C518]/30 rounded-full"></div>
                        </div>
                    ))}
                </div>
            )}

            {/* 9. ستايل بنتو (Bento Grid) */}
             {layout === 'bento_modern' && (
                <div className="grid grid-cols-4 grid-rows-2 gap-2 h-40">
                    <div className="col-span-2 row-span-2 bg-[#222] rounded-3xl border border-white/5 relative overflow-hidden flex items-end p-3">
            ...
            {layout === 'sale_grid' && (
                <div className="space-y-3">
                    <div className="w-full h-8 bg-[#F5C518] rounded-lg flex items-center justify-center text-black font-black text-[10px] uppercase">
                        Limited Time Offer
                    </div>
                    <div className="grid grid-cols-4 gap-2">
                        {[...Array(4)].map((_, i) => <div key={i} className="aspect-square bg-[#222] rounded-lg border border-white/5"></div>)}
                    </div>
                </div>
            )}
            ...
            {layout === 'circle_avatars' && (
                <div className="flex justify-center gap-4 py-2">
                    {[...Array(4)].map((_, i) => (
                        <div key={i} className="flex flex-col items-center gap-2">
                            <div className="w-12 h-12 rounded-full bg-[#222] border-2 border-white/10"></div>
            ...
            {/* 12. ستايل البانرات المستطيلة */}
            {layout === 'rect_banners' && (
                <div className="grid grid-cols-2 gap-3 h-24">
                     {[...Array(2)].map((_, i) => <div key={i} className="bg-[#222] rounded-xl border border-white/5"></div>)}
                </div>
            )}
        </div>
      </div>
            ...
            <div className="space-y-2">
                ...
                <div className="relative">
                    <select 
                        ...
                        className="w-full bg-[#1a1a1a] border border-[#333] p-4 rounded-2xl text-sm font-bold text-white focus:border-[#F5C518] transition-all outline-none appearance-none cursor-pointer"
                    >
                        ...
                        <div className="max-h-40 overflow-y-auto pr-2 space-y-2 scrollbar-hide">
                            {dataLibrary.products.map(p => (
                                <label key={p.id} className="flex items-center justify-between p-2.5 bg-[#151515] rounded-xl border border-[#333] cursor-pointer hover:border-[#F5C518]/50 transition-all">
                                    ...
                                    }} className="w-3.5 h-3.5 accent-[#F5C518]" />
                                </label>
                            ))}
                        </div>
                    </div>
