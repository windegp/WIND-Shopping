...
// استيراد إعدادات Firebase (تأكد من إنشاء ملف src/lib/firebase.js)
  ...
  <div className="flex items-center justify-between mb-6 px-4 pt-10" dir="rtl">
    <div className="flex items-center gap-3">
      ...
      <div>
        ...
        {subTitle && <p className="text-gray-400 text-[10px] md:text-xs mt-1">{subTitle}</p>}
      </div>
    </div>
...
export default function Home() {
  ...
  return (
    <main className="pb-20 bg-[#121212] min-h-screen font-sans">
      <HeroSection />
      ...
      <section className="my-12 relative h-[350px] flex items-center justify-center overflow-hidden border-y border-[#333]">
        ...
        <div className="relative z-20 text-center px-6" dir="rtl">
          ...
          <button className="bg-[#F5C518] text-black px-8 py-2.5 rounded-full font-bold text-sm hover:scale-105 transition-transform active:scale-95">
            اكتشف رحلتنا
          </button>
        </div>
      </section>
      ...
      {/* 5. قسم التقييمات (Firebase + IMDb Style) */}
      <section className="bg-[#1a1a1a] py-12 border-y border-[#333]" dir="rtl">
        <div className="max-w-[1280px] mx-auto px-4">
          <SectionHeader title="أراء وتجارب العملاء" subTitle="تقييمات موثقة من عائلة WIND" />

          {/* فورم التقييم - تصميم داكن وفخم */}
          <form onSubmit={handleSendReview} className="mb-12 max-w-2xl mx-auto bg-[#222] p-6 rounded-sm border border-[#333]">
            ...
            <div className="flex justify-between items-center flex-wrap gap-4">
              ...
              <button 
                type="submit" 
                disabled={loading}
                className="bg-[#F5C518] text-black px-10 py-2.5 font-bold text-sm uppercase hover:bg-[#ffcc00] disabled:opacity-50"
              >
          ...
          {/* عرض المراجعات بنظام الكروت السينمائية */}
              ...
              <div key={rev.id} className="min-w-[300px] md:min-w-[420px] bg-[#252525] rounded-sm overflow-hidden border border-[#333] flex snap-start">
                ...
                <div className="p-4 flex flex-col justify-center">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[#F5C518] font-bold text-lg">★ {rev.rating}/10</span>
                    <span className="text-[10px] text-green-500 font-bold uppercase tracking-widest">موثق</span>
                  </div>
                  <h4 className="text-white font-bold text-sm mb-2">{rev.userName}</h4>
                  <p className="text-gray-400 text-xs leading-relaxed italic line-clamp-3">"{rev.userComment}"</p>
                </div>
              </div>
            ))}
          </div>
          ...
          {[{ id: 1, title: "كيف تختار الشال المناسب؟", tag: "نصائح" }, { id: 2, title: "قصة الصناعة اليدوية", tag: "وراء الكواليس" }].map((art) => (
            <div key={art.id} className="group cursor-pointer">
              <div className="relative h-64 overflow-hidden rounded-sm mb-4 border border-[#333]">
                ...
                <span className="absolute top-4 right-4 z-20 bg-[#F5C518] text-black text-[10px] font-bold px-2 py-1 rounded-sm uppercase">{art.tag}</span>
              </div>
              <h3 className="text-white font-bold text-xl group-hover:text-[#F5C518] transition-colors">{art.title}</h3>
            </div>
          ))}
        </div>
      </section>
