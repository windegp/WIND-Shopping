...
export default function Home() {
  ...
  const [dynamicSections, setDynamicSections] = useState([]); // الأقسام القادمة من اللوحة
  ...
  useEffect(() => {
    ...
    // 2. الاستماع لإعدادات الصفحة (من لوحة التحكم)
    // هذا هو الجزء الذي يمنع التكرار: إذا وجدنا أقساماً هنا، سنستخدمها بدلاً من الثابت
    const unsubSettings = onSnapshot(doc(db, "settings", "homePage"), (doc) => {
        if (doc.exists() && doc.data().sections) {
            setDynamicSections(doc.data().sections);
        }
    });
  ...
  const handleSendReview = async (e) => {
    e.preventDefault();
    if (!newReview.name || !newReview.comment) return alert("الاسم والتعليق مطلوبان");
    setLoading(true);
    try {
      let url = "";
      if (newReview.image) {
  ...
  // دالة لفلترة المنتجات حسب القسم المختار في اللوحة
  const getProductsForSection = (slug) => {
    if (!slug || slug === 'all') return allProducts;
    if (slug === 'new-arrivals') return allProducts.filter(p => p.categories?.includes('new-arrivals') || p.tags?.includes('new'));
    return allProducts.filter(p => p.categories?.includes(slug) || p.category === slug);
  };

  // --- المحتوى الرئيسي (Main Render) ---
  return (
    <main className="pb-20 bg-[#121212] min-h-screen text-white relative" dir="rtl">
      
      {/* مودال التقييمات (مشترك) */}
      {isReviewModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" style={styles.modalOverlay}>
          <div className="bg-[#1A1A1A] w-full max-w-lg rounded-sm border border-[#F5C518] relative animate-[fadeIn_0.3s_ease-out] p-8">
            <button onClick={() => setIsReviewModalOpen(false)} className="absolute top-4 left-4 text-white font-bold">✕</button>
      ...
      {/* 🔥 المحرك الذكي (The Engine) 
         إذا كانت هناك بيانات من اللوحة (dynamicSections) > استخدمها.
         إذا لم تكن (أو جاري التحميل) > اعرض شاشة تحميل بسيطة أو الهيكل الافتراضي (اختياري).
         *ملاحظة: اللوحة قامت بنسخ الهيكل القديم، لذا ستظهر الصفحة كما هي تماماً.*
      */}
      {dynamicSections.length > 0 ? (
        dynamicSections.map((section, index) => {
            const products = getProductsForSection(section.collectionSlug);
            const { data } = section;
            ...
            switch (section.template) {
                ...
                case 'hero_section':
                    return (
                        <div key={section.id}>
                           {/* إذا قام الأدمن برفع صورة جديدة نعرضها، وإلا نعرض المكون الأصلي */}
                           {data?.imageUrl ? (
                    ...
                    return (
                        <section key={section.id} className="my-10">
                            ...
                            <div className="flex overflow-x-auto pb-6 px-4 gap-4 scrollbar-hide snap-x">
                                {products.slice(0, 10).map(p => <div key={p.id} className="min-w-[170px] md:min-w-[220px] snap-start hover:scale-[1.02] transition-transform"><ProductCard {...p} /></div>)}
                            </div>
                        </section>
                    );

                // 3. شريط متحرك (Marquee)
                case 'marquee':
                    return (
                        <section key={section.id} className="py-10 bg-[#161616] border-y border-[#222] overflow-hidden">
                            ...
                            <div className="relative flex overflow-x-auto scrollbar-hide" dir="ltr">
                                <div className="flex gap-6 animate-marquee-infinite pause-on-hover">
                                    {[...products.slice(0,10), ...products.slice(0,10)].map((p, i) => <div key={i} className="min-w-[200px] opacity-80 hover:opacity-100"><ProductCard {...p} /></div>)}
                                </div>
                            </div>
                        </section>
                    );

                // 4. الأكثر مبيعاً (Featured Layout)
                case 'featured':
                    return (
                        <section key={section.id} className="bg-[#181818] py-8 my-4 border-y border-[#222]">
                            ...
                            <div className="flex flex-col md:flex-row gap-6 px-4 max-w-[1400px] mx-auto">
                                {products[0] && <div className="md:w-1/3 w-full bg-[#121212] border border-[#333] p-4 relative"><div className="absolute top-4 right-4 bg-[#F5C518] text-black font-black text-xs px-2 py-1 z-10">#1</div><ProductCard {...products[0]} /></div>}
                                <div className="md:w-2/3 w-full grid grid-cols-2 gap-3">{products.slice(1, 5).map(p => <div key={p.id} className="scale-90 origin-top-right"><ProductCard {...p} /></div>)}</div>
                            </div>
                        </section>
                    );
                    ...
                    return (
                        <section key={section.id} className="bg-gradient-to-r from-[#121212] via-[#222] to-[#121212] py-8 border-y border-[#333] my-8">
                            <div className="flex justify-around items-center max-w-4xl mx-auto text-center px-4">
                               {[{v:"4.9/5",t:"تقييم العملاء"},{v:"+10k",t:"قطعة بيعت"},{v:"100%",t:"ضمان الجودة"}].map((x,i)=>(
                                   <div key={i}><h4 className="text-white text-3xl font-black">{x.v}</h4><p className="text-gray-400 text-[10px] font-bold uppercase">{x.t}</p></div>
                               ))}
                            </div>
                        </section>
                    );
                                ...
                                <div className="flex gap-6 animate-marquee pause-on-hover" dir="ltr">
                                    {reviews.length > 0 ? reviews.map((r, i) => (
                                        <div key={i} className="min-w-[300px] bg-[#121212] border border-[#333] p-6 rounded-lg">
                                            <h4 className="text-white font-black text-sm">{r.userName}</h4><p className="text-gray-400 text-sm italic">"{r.userComment}"</p>
                                        </div>
                                    )) : <p className="text-gray-500 px-4">لا توجد تقييمات بعد</p>}
                                </div>
                            </div>
                        </section>
                    );
                ...
                case 'grid':
                    return (
                        <section key={section.id} className="px-4 mb-12">
                            <SectionHeader title={section.title} subTitle={section.subTitle} link={`/collections/${section.collectionSlug}`} />
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">{products.slice(0, 8).map(p => <ProductCard key={p.id} {...p} />)}</div>
                        </section>
                    );
                                ...
                                {['dress', 'blouse'].map(cat => (
                                    <div key={cat} className="bg-[#121212] p-6 border border-[#333] relative overflow-hidden">
                                        ...
                                        <div className="flex gap-4 overflow-x-auto scrollbar-hide snap-x relative z-10" dir="ltr">
                                            {allProducts.filter(p => p.categories?.includes(cat)).slice(0,5).map(p => <div key={p.id} className="min-w-[140px]"><ProductCard {...p} /></div>)}
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                case 'winter_discounts':
                    return (
                        <section key={section.id} className="py-12 px-4">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">{allProducts.filter(p => p.compareAtPrice > p.price).slice(0, 8).map(p => <ProductCard key={p.id} {...p} />)}</div>
                        </section>
                    );
                
                // التصميمات الجديدة (IMDb, Masonry)
                default:
                    return (
        ...
        // في حالة التحميل أو عدم وجود بيانات: نعرض "شاشة تحميل أنيقة" بدلاً من تركها فارغة
        // (لاحظ أننا استغنينا عن وضع الكود المكرر هنا لأن اللوحة ستملأ البيانات فوراً)
        <div className="h-screen flex flex-col items-center justify-center bg-[#121212] text-[#F5C518]">
            <h1 className="text-4xl font-black animate-pulse">WIND</h1>
            <p className="text-gray-500 text-xs mt-2">جاري تحميل الواجهة...</p>
        </div>
      )}
    </main>
  );
